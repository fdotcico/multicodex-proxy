...
Les traces récentes de l’instance Unraid, inspectées en lecture seule, donnent :

| Route | Requêtes avec usage | Entrée p50 | Entrée p95 | Sortie p50 | Sortie p95 | Cache |
|---|---:|---:|---:|---:|---:|---:|
| `/responses` | 247 | 100 120 | 128 679 | 171 | 1 064 | médiane 98,15 % |
| `/chat/completions` | 663 | 2 931 | 7 714 | 405 | 676 | 0 % observé |

Sur l’échantillon `/responses`, 20,3 millions de tokens d’entrée contenaient
19,4 millions de tokens lus depuis le cache, soit environ 95,4 % au total.
Cette optimisation réduit surtout le coût et la latence ; les tokens restent
présents dans `usage.input_tokens`.

### Gestion actuelle des longues conversations

Les rollouts Codex locaux montrent que le client compacte déjà certaines
sessions. Sur un exemple long, l’appel de compaction a traité environ 230k
tokens non cachés pour produire une nouvelle fenêtre d’environ 19k tokens.
...
- La production Unraid n’a pas été modifiée.
