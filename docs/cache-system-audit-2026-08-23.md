...
de `http://192.168.1.149:1455/`, sans rejouer de requête vers un fournisseur et
...
La fenêtre longue disponible allait du 22 août 2026 à 12:46 UTC au 23 août à
13:23 UTC, soit environ 24 h 37. Les compteurs évoluaient pendant l'audit ; les
valeurs ci-dessous constituent une photographie cohérente à 13:23 UTC.

| Route | Requêtes | Part | Streaming | Entrée | Sortie | Latence moyenne | Succès |
|---|---:|---:|---:|---:|---:|---:|---:|
| `/chat/completions` | 81 030 | 93,6 % | 0,61 % | 420,7 M tokens | 32,3 M | 8,44 s | 99,66 % |
| `/responses` | 5 489 | 6,3 % | 100 % | 615,9 M tokens | 1,88 M | 10,44 s | 99,91 % |
| Autres | 21 | < 0,1 % | mixte | négligeable | négligeable | — | — |

Au total : 86 540 requêtes, 1,071 milliard de tokens, 288 erreurs et 99,67 %
de succès. Les deux populations sont très différentes :

- `chat/completions` est volumétriquement dominant, presque toujours non
  streamé, avec environ 5 192 tokens d'entrée et 398 de sortie par appel ;
- `responses` est moins fréquent mais représente 57,5 % des tokens, avec
  environ 112 205 tokens d'entrée et 343 de sortie par appel. Ce profil est
  caractéristique de longues sessions agentiques qui renvoient leur contexte.

Sur la rétention récente de traces complètes, les appels `Responses`
présentaient environ **93,8 % de tokens d'entrée lus depuis le cache** et aucune
écriture de cache non nulle exposée. Une analyse antérieure sur 73 386 appels
GPT-5.6 avait trouvé 96,68 % de lecture agrégée. Le mécanisme fournisseur est
donc déjà la principale économie disponible sur cette population.

Les erreurs live étaient surtout des `499`, donc des abandons client. Un cache
de réponse n'y remédie pas directement ; une idempotence explicite évite en
revanche qu'un retry déclenché après une déconnexion ne facture deux fois une
même intention.

Limites des données :

- les traces récentes sont bornées et bougent pendant la pagination ;
- les corps ne sont pas persistés, donc le taux de hit d'un cache exact ou
  sémantique n'est pas calculable a posteriori ;
- l'attribution `application` est absente sur l'historique observé, ce qui
  interdit de vérifier aujourd'hui l'isolation d'un cache par consommateur ;
- la route `/ready` du déploiement live est actuellement transmise à
  `chatgpt.com` au lieu de retourner le JSON de readiness présent sur `main` ;
  `/health` annonce encore version, commit et build à `unknown`. Le binaire
  live n'est donc pas identifiable avec certitude comme le commit audité ;
- le token d'administration live est la valeur connue `change-me`. Il doit
  être remplacé indépendamment du chantier cache.
